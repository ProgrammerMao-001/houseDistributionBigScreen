            ...
            <div class="right">
                ...
                <div class="form">
                    ...
                    <div class="main">
                        ...
                        <div class="main--user">
                            <i ref="userIcon"
                               :class="{ 'icon--chose-color': iconChose === 'username', 'icon--no-inp--color': userInpFlag.username }"
                               class="iconfont icon-user"></i>
                            <input ref="username" v-model.trim="userInfo.username"
                                   :class="{ 'inp--no-inp--color': userInpFlag.username }"
                                   @input="setInpColor('username')"
                                   @keyup="noSpace" @focus="changeColor('username', 'username')" @blur="changeColor('')"
                                   type="text" placeholder="请输入账号"/>
                        </div>
                        <!-- 输入密码 -->
                        <div class="main--psd">
                            <i ref="psdIcon"
                               :class="{ 'icon--chose-color': iconChose === 'password', 'icon--no-inp--color': userInpFlag.password }"
                               class="iconfont icon-icon-mima" style="font-size: 21px;left: 3px;top: 0;"></i>
                            <input ref="password" v-model.trim="userInfo.password"
                                   :class="{ 'inp--no-inp--color': userInpFlag.password }"
                                   @input="setInpColor('password')"
                                   @keyup="noSpace" @focus="changeColor('password', 'password')"
                                   @blur="changeColor('')"
                                   :type="isShowPwd ? 'text' : 'password'" placeholder="请输入密码"/>
                            <div @click="showPwd" style=" position: absolute; right: 30px;top: 6px;cursor: pointer;">
                                <i :class="[isShowPwd ? 'iconfont icon-yanjing' : 'iconfont icon-biyanjing',iconChose === 'password' ? 'icon--chose-color' : '', userInpFlag.password ? 'icon--no-inp--color' : '']"></i>
                            </div>
                        </div>
                        <!-- 输入验证码 -->
                        <div class="main--verifi">
                            <i ref="verificationCode"
                               :class="{ 'icon--chose-color': iconChose === 'verificationCode', 'icon--no-inp--color': userInpFlag.verificationCode }"
                               class="iconfont icon-yanzhengma"></i>
                            <input ref="verificationCode" v-model.trim="userInfo.verificationCode"
                        ...
                        <div class="rem-psd">
                            ...
                            <!-- 记住密码 -->
                            <i ref="rememberPsdRef" @click="isRememberPsd"
                               class="rem-psd_i"></i>
                            <span v-show="remPsdFlag"
                                  style="font-size: 20px; line-height: 0px; color: #f08080;"> √ </span>
                            <div @click="isRememberPsd" class="rem-psd_div">记住密码</div>
                        </div>
                        <!-- 登录按钮 -->
                        <el-button @click="handleLogin" :loading="isLoading" type="primary" class="btn--login">登录
                        </el-button>
                    </div>
                    <!-- 登录尾部 -->
                    <div class="footer">
...
<script>
    import {getPicCode, login} from "@/api/loginModule";
    ...
    export default {
        ...
        methods: {
             ...
             */
            getUserInfo() {
                let helpUserInfo = JSON.parse(localStorage.getItem('userInfo'));
                let helpRemPsdFlag = JSON.parse(localStorage.getItem('remPsdFlag'));
             ...
             * @returns 无
             */
            changeColor(iconChose, type) {
                ((this.userInpFlag)[type] = false)
            ...
            handleLogin() {
                ...
                for (const key in this.userInfo) {
                    if ((this.userInfo)[key] == '') {
                        ...
                        (this.userInpFlag)[key] = true;
                    }
                }
                // 判断验证码是否正确
                if (flag && this.userInfo.verificationCode !== this.verificationCodeKey) {
                    (this.userInfo).verificationCode = ''; // 清空内容
                    (this.$refs.verificationCode).focus(); // 聚焦
                    (this.userInpFlag).verificationCode = true; // 红色提示
                    this.tipsMark = true;
                    this.tips = '请输入正确验证码';
                    this.isLoading = false;
                    this.getValidatorCode(); // 刷新验证码
                }
                if (flag && flag && this.userInfo.verificationCode === this.verificationCodeKey) {
                    this.isLoading = true;
                    this.tipsMark = false;
                    login({
                        ...
                        if (res.status === 101404) {
                            ...
                            (this.userInpFlag).username = true; // 红色提示
                            this.tipsMark = true;
                            this.tips = '账号不存在，请重试！';
                            this.isLoading = false;
                        }
                        if (res.status === 101101) {
                            ...
                            (this.userInpFlag).password = true; // 红色提示
                            this.tipsMark = true;
                            this.tips = '密码错误，请重试！';
                            this.isLoading = false;
                        }
